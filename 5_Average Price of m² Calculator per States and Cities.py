...
for row in range(2, sheet.max_row + 1):
    try:
        ...
        area = sheet.cell(row=row, column=5).value
        ...
        if price is not None and area is not None and area != 0:
            price_per_m2 = float(price.replace(" ","")) / float(area)
...
for state, total in state_totals.items():
    ...
    states_sheet[f'A{row_num}'] = state # State name
...
for city, total in city_totals.items():
    ...
    cities_sheet[f'B{row_num}'] = city_info[1]  # State name
